...
$(document).ready(function(){
	...
	socket.on('if_online' , (data)=>{
      ...
      }else if ( data.type == 'contractor' && active_contractors.indexOf([data.user_id]) < 0 ) {
      
      	active_contractors.push(data.user_id); 	
      
      }

      $('.number_of_active_suppliers').html( active_suppliers.length );
      $('.number_of_active_contractors').html( active_contractors.length );
		 ...
		 chatPopup =  `<div class="popup-box box-${uid} chat-popup popup-box-on" id="qnimate" rel="${uid}">
    		  <div class="popup-head">
				<div class="popup-head-left pull-left">
					<img class="img-responsive img-thumbnail" width="100px" height="100px;" src="${imageUrl}" alt="iamgurdeeposahan"> 
			...
			<label style="padding-left: 10px;cursor:pointer"><input type="file" multiple='' accept="image/*" style="display:none;" id="image_chat"><i class="fa fa-image"></i></label>
		 ...
		 $.ajax({
			...
			Object.keys(response).forEach( function(key){
				...
				if (message.type =='image') {
					...
					file = `<a href="${message.content}"> <img width="100px" height="100px" src="${message.content}"> </a>`;

				}else if (message.type =='file') {
					file = `<a href="${message.content}" > ${message.content.split('/')[1]} </a>`;			
				}else{
					file = message.content;
				}

				var sender_name = message.sender.full_name ;
	...
	socket.on('new_private_message', (data) => {
		...
		if (flag == false) {
			...
			var message_body = `<div class="direct-chat-msg doted-border">
		                      ...
		                      <div class="direct-chat-text message clearfix" style="width: 82%;">
		                      	${data.message}
		                      </div>
	...
	socket.on('new_private_file', (data) => {
		...
		if(sender == user_id){
			...
			.done(function(response) {
				...
				if (type=='image') {
					$('.uploaded_image' + i).attr('href', `/upload/${link}`);
				}else{
					$('.uplaoded_file'+i).attr('href', `/upload/${link}`);
				}
		...
		if (data.type =='image') {
			...
			file = `<a style="display: table; width: 83%;" href="${data.file}" class="uploaded_image${i}"> <img class="img-thumbnail" width="100px" height="100px" src="${data.file}"> </a>`;

		}else{
			file = `<a style="display: table; width: 83%;" href="${data.file}" class="uplaoded_file${i}" > ${data.filename} </a>`;			
		}
		...
		if ( flag == false ) {
			...
			var message_body = `<div class="direct-chat-msg doted-border">
			                      ...
			                      <!-- /.direct-chat-info -->
			                      <img alt="message user image" src="${imageUrl}" class="direct-chat-img"><!-- /.direct-chat-img -->
			                      <div class="direct-chat-text message" style="display: table;width: 83%;">
			                         ${file}
			                      </div>
	...
	$(document).on('change' , '#image_chat' , function(e) {
	...
	$(document).on('change' , '#file_chat' , function(e) {
